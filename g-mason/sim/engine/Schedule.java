...
public class Schedule implements java.io.Serializable
    ...
    /** The parameter for reduce  memory copy overhead between GPU and CPU 
     * It means that the actual steps will increase by gpuSteps every time the schedule can complete one step on CPU side
     * Set this value high may cause the Graphic presentation be glitched. But It's useful when you want to skip more steps ahead
     * and when you are only run the model on console.
     **/
    public int gpuSteps = 1;     //how many steps that we run on GPU before copy data back to CPU
   
	
    //for concurrent gpu thread
    public final Object  concurrent = new Object();
    ...
    public void setgpuSteps(int gpuSteps)
    {
		this.gpuSteps = gpuSteps;
		System.out.println("value changed "+gpuSteps);
	}
    
    public int getgpuSteps()
    {
		return gpuSteps;
	}    
    ...
    */
    ...
    public synchronized boolean step(final SimState state)
		...
		//return true;//waiting until gpu complete, efficient ?
	...
	//	long test_start_time = System.nanoTime();
        ...
        try
            ...
            synchronized(lock) { /*steps++; gpuSteps = 1 if not set*/ steps += gpuSteps;}
            inStep = false;
            
    //        long test_end_time = System.nanoTime();
	//		System.out.println("phase 3: " + (test_end_time - test_start_time));
			 if (currentPhase ==3)
			{
					currentPhase = 2;
			}
