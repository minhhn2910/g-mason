...
public class SimState implements java.io.Serializable
    ...
    public static int numGPUStepsPerCopyBack = 1;
    public int getnumGPUStepsPerCopyBack() { return numGPUStepsPerCopyBack; }
    public void setnumGPUStepsPerCopyBack(int val) { if (val >= 1) numGPUStepsPerCopyBack = val; } 
    ...
    */
    ...
    public void start()
        {
		schedule.setgpuSteps(numGPUStepsPerCopyBack);	
        // just in case
        cleanupAsynchronous();
        // reset schedule
        schedule.reset();
    ...
    public static void doLoop(MakesSimState generator, String[] args)
        ...
        if (keyExists("-help", args))
            ...
            System.err.println(
				...
				"-gpusteps I       Positive int value not 0: number of GPU steps before \n" +
                "                  copy all data back to host(CPU) for synchronize and display.\n" +
                "                  Default: 1.\n\n" +                
        ...
        if (!quiet) System.err.println("G-MASON Version " + n.format(version()) + " Optimized for GPU.  For further options, try adding ' -help' at end.");
        ...
        int gpu_steps = 1;
        String gpu_steps_s = argumentForKey("-gpusteps", args);
        if (gpu_steps_s != null)
            try
                {
                gpu_steps = Integer.parseInt(gpu_steps_s);
                numGPUStepsPerCopyBack=gpu_steps;
                if (gpu_steps == 0) throw new Exception();
                }
            catch (Exception e)
                {
                throw new RuntimeException("Invalid 'gpusteps' value: " + gpu_steps + ", must be a non-zero positive integer, or nonexistent");
                }
                ...
                if (time < 0)  // don't know how long to make the time yet
                    {
						if(steps<=1)
						{
							oldClock = System.currentTimeMillis();
						}
						else if (System.currentTimeMillis() - oldClock >1500L)  // time to set the time
							{
