...
//                {
            ...
            if (newBuffer || !immutableField || isDirtyField())  // we have to load the buffer
                ...
                if (ex-sx > 0 && ey-sy > 0)  // could be otherwise if drawing off-screen...
                    ...
                    if (isDoubleGrid2D)
                        for(int y=sy;y<ey;y++)
                            for(int x=sx;x<ex;x++)
                                data[i++] = map.getRGB(/*doubleField[x][y]*/((DoubleGrid2D)field).get(x,y));  //modify for doubleGrid2Donly Further need sample model to test
                    else
                        for(int y=sy;y<ey;y++)
                            for(int x=sx;x<ex;x++)
            ...
            if (isDoubleGrid2D)
                for(int x=sx;x<ex;x++)
                    for(int y=sy;y<ey;y++)
                        {
                        final Color c = map.getColor(/*doubleField[x][y]*/((DoubleGrid2D)field).get(x,y));
            ...
            if (isDoubleGrid2D)
                for(double x=startxd;x<endxd;x++)
                    for(double y=startyd;y<endyd;y++)
                        {
                        final Color c = map.getColor(/*doubleField[(int)x][(int)y]*/((DoubleGrid2D)field).get((int)x,(int)y));
                        if (c.getAlpha() == 0) continue;
                        graphics.setColor(c);
