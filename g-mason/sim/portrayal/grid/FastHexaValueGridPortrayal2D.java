                    ...
                    for(int x=sx;x<ex;x++)
                        for(int y=sy;y<ey;y++)
                            ...
                            if((x&1)==0)
                                ...
                                load = map.getRGB(((DoubleGrid2D)field).get(x,y));
...
//                              _raster.setDataElements(x,2*y+2,load);
            ...
            if (isDoubleGrid2D)
                for(int x=sx;x<ex;x++)
                    for(int y=sy;y<ey;y++)
                        {
                        final Color c = map.getColor(((DoubleGrid2D)field).get(x,y));
                        ...
                        if( graphics!=null )
                            ...
                            if( info.clip.intersects(_x,_y,_width,_height) )
                                putInHere.add(getWrapper((((DoubleGrid2D)field).get(x,y)), new Int2D(x, y)));
                            }
                        }
            else
                for(int x=sx;x<ex;x++)
            ...
            if (isDoubleGrid2D)
                for(double x=startxd;x<endxd;x++)
                    for(double y=startyd;y<endyd;y++)
                        {
                        final Color c = map.getColor(((DoubleGrid2D)field).get((int)x,(int)y));
                        if (c.getAlpha() == 0) continue;
                        graphics.setColor(c);
